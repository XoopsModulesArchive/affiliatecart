...
function affiliatecart()
{
	...
	// Temporarily 'homeless' links (to be revised in admin.php breakup)
		$result = $xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_broken")."");
	...
	if($totalbrokenlinks>0){
		...
		$result2 = $xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_mod")."");
	...
	$result3 = $xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_links")." where status=0");
	...
	$result=$xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_links")." where status>0");
...
function listNewLinks()
{
	...
	$linkimg_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH."/modules/affiliatecart/images/shots/");
    $result = $xoopsDB->query("select lid, cid, title, url, logourl, submitter from ".$xoopsDB->prefix("affiliatecart_links")." where status=0 order by date DESC");
    ...
    if ( $numrows > 0 ) {
		while(list($lid, $cid, $title, $url, $logourl, $submitterid) = $xoopsDB->fetchRow($result)) {
			...
			$shotdir = "<b>".XOOPS_URL."/modules/affiliatecart/images/shots/</b>";
...
function linksConfigMenu()
{
    ...
    $result=$xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_cat")."");
	list($numrows)=$xoopsDB->fetchRow($result);
    if ( $numrows > 0 ) {
	echo"<table width='100%' border='0' cellspacing='1' class='outer'>"
	."<tr class=\"odd\"><td>";
	...
	// Modify Link
    $result2 = $xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_links")."");
	...
	."<tr class=\"odd\"><td>";
		...
		echo "<input type=hidden name=fct value=affiliatecart>\n";
...
function modLink()
{
	...
	echo"<table width='100%' border='0' cellspacing='1' class='outer'>"
	."<tr class=\"odd\"><td>";
   	$result = $xoopsDB->query("select cid, title, url, logourl from ".$xoopsDB->prefix("affiliatecart_links")." where lid=$lid") or $eh->show("0013");
   	echo "<h4>"._MD_MODLINK."</h4><br />";
    ...
    $result5=$xoopsDB->query("SELECT count(*) FROM ".$xoopsDB->prefix("affiliatecart_votedata")." WHERE lid = $lid");
    ...
    $result5=$xoopsDB->query("SELECT ratingid, ratinguser, rating, ratinghostname, ratingtimestamp FROM ".$xoopsDB->prefix("affiliatecart_votedata")." WHERE lid = $lid AND ratinguser >0 ORDER BY ratingtimestamp DESC");
    ...
    while(list($ratingid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp)=$xoopsDB->fetchRow($result5)) {
    	...
    	$result2=$xoopsDB->query("SELECT rating FROM ".$xoopsDB->prefix("affiliatecart_votedata")." WHERE ratinguser = '$ratinguser'");
    ...
    $result5=$xoopsDB->query("SELECT ratingid, rating, ratinghostname, ratingtimestamp FROM ".$xoopsDB->prefix("affiliatecart_votedata")." WHERE lid = $lid AND ratinguser = 0 ORDER BY ratingtimestamp DESC");
    $votes = $xoopsDB->getRowsNum($result5);
...
function delVote()
{
    ...
    $lid = $HTTP_GET_VARS['lid'];
	$sql = sprintf("DELETE FROM %s WHERE ratingid = %u", $xoopsDB->prefix("affiliatecart_votedata"), $rid);
...
{
   	global $xoopsDB, $eh;
   	$result = $xoopsDB->query("select * from ".$xoopsDB->prefix("affiliatecart_broken")." group by lid order by reportid DESC");
	...
	echo "<h4>"._MD_BROKENREPORTS." ($totalbrokenlinks)</h4><br />";
   	...
   	if ( $totalbrokenlinks == 0 ) {
        ...
        while ( list($reportid, $lid, $sender, $ip)=$xoopsDB->fetchRow($result) ) {
			$result2 = $xoopsDB->query("select title, url, submitter from ".$xoopsDB->prefix("affiliatecart_links")." where lid=$lid");
			if ( $sender != 0 ) {
...
function delBrokenLinks()
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
    $lid = $HTTP_GET_VARS['lid'];
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_broken"), $lid);
    $xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_links"), $lid);
    $xoopsDB->query($sql) or $eh->show("0013");
...
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_broken"), $HTTP_GET_VARS['lid']);
    $xoopsDB->query($sql) or $eh->show("0013");
...
function listModReq()
{
    ...
    $result = $xoopsDB->query("select requestid,lid,cid,title,url,logourl,description,modifysubmitter from ".$xoopsDB->prefix("affiliatecart_mod")." order by requestid");
	...
	if ( $totalmodrequests > 0 ) {
		...
		while ( list($requestid, $lid, $cid, $title, $url, $logourl, $description, $submitterid)=$xoopsDB->fetchRow($result) ) {
			...
			$result2 = $xoopsDB->query("select cid, title, url, logourl, submitter from ".$xoopsDB->prefix("affiliatecart_links")." where lid=$lid");
			...
			if ( $xoopsModuleConfig['useshots'] && !empty($origlogourl) ) {
				echo "<img src=\"".XOOPS_URL."/modules/affiliatecart/images/shots/".$origlogourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\" />";
			} else {
				echo "&nbsp;";
			}
			echo "</td></tr>
			...
			if ( $xoopsModuleConfig['useshots'] == 1 && !empty($logourl) ) {
				echo "<img src=\"".XOOPS_URL."/modules/affiliatecart/images/shots/".$logourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\" alt=\"/\" />";
			} else {
				echo "&nbsp;";
			}
			echo "</td></tr>
...
function changeModReq()
{
    ...
    $query = "select lid, cid, title, url, logourl, description from ".$xoopsDB->prefix("affiliatecart_mod")." where requestid=".$requestid."";
    ...
    while ( list($lid, $cid, $title, $url, $logourl, $description)=$xoopsDB->fetchRow($result) ) {
		...
		$sql= sprintf("UPDATE %s SET cid = %u, title = '%s', url = '%s', logourl = '%s', status = 2, date = %u WHERE lid = %u", $xoopsDB->prefix("affiliatecart_links"), $cid, $title, $url, $logourl, time(), $lid);
    	$xoopsDB->query($sql) or $eh->show("0013");
		$sql = sprintf("UPDATE %s SET description = '%s' WHERE lid = %u", $xoopsDB->prefix("affiliatecart_text"), $description, $lid);
		$xoopsDB->query($sql) or $eh->show("0013");
		$sql = sprintf("DELETE FROM %s WHERE requestid = %u", $xoopsDB->prefix("affiliatecart_mod"), $requestid);
		$xoopsDB->query($sql) or $eh->show("0013");
    }
...
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
	$sql = sprintf("DELETE FROM %s WHERE requestid = %u", $xoopsDB->prefix("affiliatecart_mod"), $HTTP_GET_VARS['requestid']);
    $xoopsDB->query($sql) or $eh->show("0013");
...
function modLinkS()
{
    ...
    $xoopsDB->query("update ".$xoopsDB->prefix("affiliatecart_links")." set cid='$cid', title='$title', url='$url', logourl='$logourl', status=2, date=".time()." where lid=".$HTTP_POST_VARS['lid']."")  or $eh->show("0013");
    $xoopsDB->query("update ".$xoopsDB->prefix("affiliatecart_text")." set description='$description' where lid=".$HTTP_POST_VARS['lid']."")  or $eh->show("0013");
    redirect_header("index.php",1,_MD_DBUPDATED);
	exit();
}
...
function delLink()
{
	...
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_links"), $HTTP_GET_VARS['lid']);
   	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_text"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_votedata"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
	// delete comments
...
function modCat()
{
	...
	$result=$xoopsDB->query("select pid, title, imgurl from ".$xoopsDB->prefix("affiliatecart_cat")." where cid=$cid");
...
function modCatS()
{
	...
	$xoopsDB->query("update ".$xoopsDB->prefix("affiliatecart_cat")." set pid=$pid, title='$title', imgurl='$imgurl' where cid=$cid") or $eh->show("0013");
    redirect_header("index.php",1,_MD_DBUPDATED);
}
...
function delCat()
{
    ...
    if ( $ok == 1 ) {
		...
		for ( $i=0;$i<$dcount;$i++ ) {
			...
			$result=$xoopsDB->query("select lid from ".$xoopsDB->prefix("affiliatecart_links")." where cid=".$arr[$i]."") or $eh->show("0013");
			//now for each link, delete the text data and vote ata associated with the link
			while ( list($lid)=$xoopsDB->fetchRow($result) ) {
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_text"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_votedata"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_links"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
			...
			$sql = sprintf("DELETE FROM %s WHERE cid = %u", $xoopsDB->prefix("affiliatecart_cat"), $arr[$i]);
			$xoopsDB->query($sql) or $eh->show("0013");
		}
		//all subcategory and associated data are deleted, now delete category data and its associated data
		$result=$xoopsDB->query("select lid from ".$xoopsDB->prefix("affiliatecart_links")." where cid=".$cid."") or $eh->show("0013");
		while ( list($lid)=$xoopsDB->fetchRow($result) ) {
			$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_links"), $lid);
			$xoopsDB->query($sql) or $eh->show("0013");
			$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("affiliatecart_text"), $lid);
		...
		$sql = sprintf("DELETE FROM %s WHERE cid = %u", $xoopsDB->prefix("affiliatecart_cat"), $cid);
	    $xoopsDB->query($sql) or $eh->show("0013");
...
function addCat()
{
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("affiliatecart_cat")."_cid_seq");
	$sql = sprintf("INSERT INTO %s (cid, pid, title, imgurl) VALUES (%u, %u, '%s', '%s')", $xoopsDB->prefix("affiliatecart_cat"), $newid, $pid, $title, $imgurl);
	$xoopsDB->query($sql) or $eh->show("0013");
	if ($newid == 0) {
...
function addLink()
{
    ...
    $result = $xoopsDB->query("select count(*) from ".$xoopsDB->prefix("affiliatecart_links")." where url='$url'");
    list($numrows) = $xoopsDB->fetchRow($result);
	$errormsg = "";
	$error = 0;
    if ( $numrows > 0 ) {
	...
	$sql = sprintf("INSERT INTO %s (lid, cid, title, url, logourl, submitter, status, date, hits, rating, votes, comments) VALUES (%u, %u, '%s', '%s', '%s', %u, %u, %u, %u, %u, %u, %u)", $xoopsDB->prefix("affiliatecart_links"), $newid, $cid, $title, $url, $logourl, $submitter, 1, time(), 0, 0, 0, 0);
	$xoopsDB->query($sql) or $eh->show("0013");
	if ( $newid == 0 ) {
		$newid = $xoopsDB->getInsertId();
	}
...
function approve()
{
	...
	$query = "update ".$xoopsDB->prefix("affiliatecart_links")." set cid='$cid', title='$title', url='$url', logourl='$logourl', status=1, date=".time()." where lid=".$lid."";
	$xoopsDB->query($query) or $eh->show("0013");
	$query = "update ".$xoopsDB->prefix("affiliatecart_text")." set description='$description' where lid=".$lid."";
	$xoopsDB->query($query) or $eh->show("0013");
	global $xoopsModule;
	$tags=array();
    $tags['LINK_NAME'] = $title;
